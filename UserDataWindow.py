...
class UserDataWindow(QWidget):
    ...
    def setup(self):
        self.setWindowTitle('Claimed By')
    ...
    def generate_line(self, field: int, label_text: str, label_x: int, label_y: int, line_x: int, line_y: int, width=None):
        claim_record = get_claim_record('cubes_db.sqlite', self.db_entry)
        # print(claim_record)
        if len(claim_record) != 0:
            self.user_record = get_user_record('cubes_db.sqlite', claim_record[0][1])
